...
Αρχικά πέρνουμε σε μια μεταβλητή 'uuid' το αποτέλεσμα της συνάρτησης 'request.headers.get('authorization')' με παράμετρο ''authorization'' όπου ουσιαστικά πέρνει τον κωδικό του authorization που έχει δημιουργηθεί απο το 'login'. Έπειτα μέσα σε μία δομή επιλογής και μέσα απο την συνάρτηση 'is_session_valid(uuid)' με παράμετρο την μεταβλητή 'uuid' επαληθεύει τον 'user'. Στην περίπτωση που δεν επαληθευτεί γυρνάει μήνυμα λάθους.  Στην περίπτωση που βρεθεί το email που έχει δωθεί απο τον χρήστη θα εισαχθεί στην μεταβλητή iterable τα δεδομένα του φοιτητή διαφορετικά θα επιστρέφει μήνυμα λάθους. Μετά θα ελέγχει άμα η μεταβλητή iterable έχει τα δεδομένα της αναζήτησης και εισάγει στο student το όνομα του φοιτητή (κανονικά και την διευθηνση αλλά δεν κατάφερα να το κάνω να λειτουργήσει άρα το αφαίρεσα για να μπορεί να τρέξει). Τέλος γυρνάει το student, σε διαφορετική περίπτωση γυρνάει μήνυμα λάθους. 
...
Αρχικά πέρνουμε σε μια μεταβλητή 'uuid' το αποτέλεσμα της συνάρτησης 'request.headers.get('authorization')' με παράμετρο ''authorization'' όπου ουσιαστικά πέρνει τον κωδικό του authorization που έχει δημιουργηθεί απο το 'login'. Έπειτα μέσα σε μία δομή επιλογής και μέσα απο την συνάρτηση 'is_session_valid(uuid)' με παράμετρο την μεταβλητή 'uuid' επαληθεύει τον 'user'. Στην περίπτωση που δεν επαληθευτεί γυρνάει μήνυμα λάθους. Ελέγχει άμα το email υπάρχει στο collection Students και βάζει στην μεταβλητή name τα δεδομένα. Τα διαγράφει και γυρνάει το όνομα του. Διαφορετικά γυρνάει μήνυμα λάθους.
